...
本轮试用以浅色蓝白灰作为默认界面色面，优先级高于下方旧版 Dark / Light token。目标是让桌面、聊天、设置、联系人、地图、离线页和 PWA 外壳统一到同一组冷白色纸面，便于整次提交回退。
...
- `#FFFFFF` 只作为页面与屏幕基底。
- `#F7FCFF` 用于普通纸面、列表行和轻容器。
- `#EEF7FE` 用于输入框、聊天 AI 气泡和次级面板。
- `#E7F4FC` 用于桌面背景、hover、地图水面和轻量强调区域。
- `#E2F2FC` 用于选中前的柔和填充、头像底和地图陆地。
- `#C8D3DA` 用于边框、当前态、发送按钮和低饱和分隔。
...
`live2love` 的手机端尺寸以 Apple Human Interface Guidelines 的 44pt 触控目标作为下限。落到本项目的 Web/PWA 实现中，按 1x viewport 处理为 **44 CSS px**。

执行规则：

- 所有可点击控件、可点头像、底部导航项、返回按钮、图标按钮、筛选 chip、分组 chip 和表单按钮，可点击区域不得小于 44px × 44px。
- 图标本体可以保持 18px 至 24px，但外层按钮必须提供 44px 以上的命中区域。
- 输入框、select、数字输入、搜索框高度不得低于 44px；聊天输入框建议 48px。
- 开关的视觉轨道可以小于 44px，但承载它的 label/row 必须至少 44px 高，并且整行可点。
- 聊天正文使用 17px；普通列表主文本 16px 至 17px；11px 至 12px 只用于非交互的辅助信息、徽章或时间。
- 可视 home handle 可以保持细条形态，但按钮命中区域必须至少 44px 高。
- 如果用户明确要求沉浸聊天或释放顶部空间，可以隐藏自定义顶部状态栏；不要为了模拟系统而牺牲正文、输入框和返回路径的可用面积。
...
- 顶部状态栏不是强制层。需要系统状态时可显示时间、网络、API 状态或离线状态；聊天等沉浸场景可隐藏自定义状态栏，把空间留给标题、消息和输入区。
